...
class couples
{
	public:
		...
		int pairingq1(boys *boyss,girls *girlss,int nb,int ng)		//!Pairing.
		{
		...
		int pairingq5(boys *boyss,girls *girlss,int nb,int ng)		//!Pairing.
		{
			FILE *fc = fopen("couples.txt","w+");
			for(int k=0;k<2*nb;k++)
			{
				//printf("%s\n",girlss[i].name.c_str());
				int i = k/2;
				if(k%2 == 0 && i<ng)	//!First a girl is given a chance.
				{
					//std::cout<<girlss[i].name<<" "<<i<<"\n";
					if(girlss[i].committed==0)
					{
						if(girlss[i].need.compare("Attractive")==0)
						{
					...
					//std::cout<<girlss[i].committed<<"\n";
				}
				else
				{
					//std::cout<<boyss[i].name<<" "<<i<<"\n";
					//!The boy is given a chance here.
					if(boyss[i].committed == 0)
					{
						int min_att=0,minind=-1;
						for(int j=0;j<ng;j++)
						{
							if(girlss[j].committed == 0 && boyss[i].min_attractive<=girlss[j].attractiveness && boyss[i].budget>=girlss[j].maintenance && girlss[j].attractiveness>=min_att)
							{
								min_att = girlss[j].attractiveness;
								minind = j;
							}
						}
						if(minind!=-1)
						{
							girlss[minind].committed = 1;
							boyss[i].committed = 1;
							boyss[i].girlname = girlss[minind].name;
							girlss[minind].boyname = boyss[i].name;
							fprintf(fc,"%s %s %s %s %d %d %d %d %d %d\n",boyss[i].name.c_str(),boyss[i].type.c_str(),girlss[minind].name.c_str(),girlss[minind].type.c_str(),boyss[i].budget,girlss[minind].maintenance,boyss[i].attractiveness,girlss[minind].attractiveness,boyss[i].intelligence,girlss[minind].intelligence);
						}
					}
					//std::cout<<boyss[i].committed<<"\n";
				}
			}
			fclose(fc);
			return 0;
		...
		int mosthappy(couples* couple,int count,int k)	//!bubble sort for happiness.
		{
			for(int i=0;i<count;i++)
			{
				for(int j=0;j<count-1;j++)
				{
					...
					{
						std::swap(couple[j],couple[j+1]);
					}
				}
			}
			std::cout<<"K happiest couples "<<"\n";
			for(int i=0;i<count;i++)
				std::cout<<couple[i].bname<<" and "<<couple[i].gname<<std::endl;
			return 0;
		}
};
