...
class couples
{
	public:
				...
				if(girlss[i].committed==0)
				{
					if(girlss[i].need.compare("Attractive")==0)
					{
						...
						if(minind!=-1)
						{
							int ss;
							ss = v.size();
							minind = v[rand()%ss];	//!Selecting random among sutable boys for a girl.
							//cout<<minind<<endl;
							v.clear();
		...
		int couplegifting(couples* couple,int count,gifts* gif,int ngf)	//!Gift Exchanges.
		{
			...
			for(int i=0;i<count;i++)		//!Using the old gifting technique, just adding the random condition.
			{
				printf("\n");
				int tl=0,tc=0,tv=0;
				if(couple[i].btype.compare("Miser")==0)
				{
					int selected[ngf],count=0;
					for(int j=0;j<ngf;j++)
						selected[j] = 0;
					while(1)
					{
						if(count == ngf)			//! If count equals total number of gifts, loop breaks.
							break;
						count++;
						int j = rand()%ngf;
						if(selected[j] == 1)
