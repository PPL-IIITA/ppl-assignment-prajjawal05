...
class couples
{
	public:
		...
		int mosthappy(couples* couple,int count,int k)	//!bubble sort for happiness.
		{
			...
			gi.input(girlss,ng);
			for(int i=0;i<nb;i++)	//!Reading boys data.
			{
				boyss[i].girlname="";
			...
			for(int i=0;i<ng;i++)	//!Reading Girls data.
			{
				girlss[i].boyname="";
				for(int j=0;j<count;j++)
				{
			...
			for(int i=count-1;i>count-k;i--)	//!Loop to make breakups if in least k happiest couples.
			{
				for(int j=0;j<nb;j++)
				{
			...
			printf("Relationships after breakups\n");
			FILE *fc = fopen("New Couples.txt","w+");
			for(int i=0;i<ng;i++) //!Printing the new Relationships.
			{
				for(int j=0;j<nb;j++)
				{
					if(boyss[j].girlname.compare(girlss[i].name) == 0 && girlss[i].boyname.compare(boyss[j].name) == 0 &&girlss[i].committed == 1 && boyss[j].committed == 1)	//! They shouldn't be in a relationship prevoiusly.
					{
