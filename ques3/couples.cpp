	...
	public:
		...
		int pairing(boys *boyss,girls *girlss,int nb,int ng)		//!Pairing.
		{
			...
			for(int i=0;i<ng;i++)
			{
				...
				{
					if(girlss[i].need.compare("Attractive")==0)	//!checking the girl type if not committed;
					{
		...
		int mosthappy(couples* couple,int count,int k)	//!bubble sort for happiness.
		{
			...
			printf("\nMost Happiest:\n");
			for(int i=0;i<k;i++)
				printf("%s and %s\n",couple[i].bname.c_str(),couple[i].gname.c_str());
				return 0;
		}
		int mostcompatible(couples* couple,int count,int k) //!bubble sort for compatibility.
		{
			for(int i=0;i<count;i++)
			{
				for(int j=0;j<count-1;j++)
				{
					if(couple[j+1].compatibility>couple[j].compatibility)
					{
						std::swap(couple[j+1].happiness,couple[j].happiness);
						std::swap(couple[j+1].compatibility,couple[j].compatibility);
						couple[j].bname.swap(couple[j+1].bname);
