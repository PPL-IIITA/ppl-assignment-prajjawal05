...
class couples
{
	public:
		...
		int couplegiftingnew(couples* couple,int count,gifts* gif,int ngf)	//!Gift Exchanges.
		{
			std::ofstream output;
			output.open("log.txt", std::ios_base::app);
			FILE *fcc = fopen("fcalc.txt","w+");
			for(int i=0;i<count;i++)
			{
				int selected[ngf];	//!keeping an hash table to keep track of all the gist selected.
				for(int j=0;j<ngf;j++)
					selected[j] = 0;
				printf("\n");
				int tl=0,tc=0,tv=0;
				for(int j=0;j<ngf;j++)
				{
					if(selected[j]==0 && gif[j].type.compare("Luxury")==0)	//!alloting one Luxury Gift.
					{
						time_t now = time(0);
						char *dt = ctime(&now);
						tl+=gif[j].price;
				...
				for(int j=0;j<ngf;j++)
				{
					if(selected[j]==0 && gif[j].type.compare("Essential")==0)	//Alloting one essential gift.
					{
						time_t now = time(0);
						char *dt = ctime(&now);
						tl+=gif[j].price;
				...
				for(int j=0;j<ngf;j++)
				{
					if(selected[j]==0 && gif[j].type.compare("Utility")==0)	//Alloting one Utility Gift.
					{
						time_t now = time(0);
						char *dt = ctime(&now);
						tl+=gif[j].price;
				...
				//Using Old gifting method here onwards for different type of boys and girls comprising  a couple and calculating their happiness level and compatibility.
				if(couple[i].btype.compare("Miser")==0)
				{
					for(int j=0;j<ngf;j++)
					{
